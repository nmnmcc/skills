...
- `pattern` turns recurring project experience into independent, runnable teaching repositories under `patterns/`. Each `patterns/<pattern-name>/` child is its own repository and teaches one pattern through a working reference implementation, verified commands, a guided experiment, and the situation, tensions, response, consequences, and boundary that make the lesson transferable.
