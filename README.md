...
- `effect` makes computational effects, dependencies, failures, resources, control flow, and execution boundaries explicit. For Algebraic Effects, it also makes operations, handlers, and resumptions explicit. It keeps supported work inside the project's effect system and requires explicit user approval before an unavoidable escape hatch.
